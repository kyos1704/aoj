...
bool inside(P a){
  return -EPS<a.X&&a.X<50+EPS&&-EPS<a.Y&&a.Y<94+EPS;
}
...
double solve(){
  ...
  for(int i=0;i<in.size();i++){
    vector<L> tmp = TLine_CP(in[i],s);
    for(auto i:tmp){
      sl.push_back(i);
    }
  }
  for(int i=0;i<in.size();i++){
    vector<L> tmp = TLine_CP(in[i],g);
    for(auto i:tmp){
      gl.push_back(i);
    }
  }
  double ans = INF;
  {
    int c = 0;
    for(auto i:in){
      if(isCrossCS(i,L(s,g)))c++;
    }
    if(c<=d)ans= min(ans,distPP(s,g));
  }
  for(auto i:sl){
    for(auto j:gl){
      if(!isCrossLL(i,j))continue;
      P tmp = crossP_LL(i,j);
      if(!inside(tmp))continue;
      pair<S,S> route(L(s,tmp),L(tmp,g));
      int c = 0;
      for(auto i:in){
        if(isCrossCS(i,route.first))c++;
        if(isCrossCS(i,route.second))c++;
        if(isCrossCP(i,tmp))c--;
      }
      if(c>d)continue;
      ans = min(distPP(route.first[0],route.first[1])+distPP(route.second[0],route.second[1]),ans);
    }
  }
